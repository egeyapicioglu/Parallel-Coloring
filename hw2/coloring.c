...
int coloring(etype * row_ptr, vtype * col_ind, int nov)
{
  ...
  while (boolean)
  {
    ...
    int forbidden_array[nov][nov];
    printf("Burda mi\n");
    for (i = 0; i < nov; i++)
    {
      for(y = 0; y < nov; y++)
      {
        forbidden_array[i][y] = 0;
      }
    }
    printf("Yoksa burda mi? \n");
    start = omp_get_wtime();
    #pragma omp parallel for shared(color, forbidden_array) num_threads(THREAD_NUMBER) schedule(guided) // Coloring part
